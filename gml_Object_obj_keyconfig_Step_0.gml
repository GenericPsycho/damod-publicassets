    ...
    if keyboard_check_pressed(vk_anykey)
    {
        if (keyboard_key != vk_f1)
        {
            if ((!menu) || scr_check_menu_repeats(input[key_select][0], keyboard_key, false))
            {
                ...
                if (!(in.has_value((0 << 0), keyboard_key)))
                    array_push(in.actions, new tdp_input_action((0 << 0), keyboard_key))
                selecting = false
            }
...
{
    if (1 && keyboard_check_pressed(vk_anykey))
    {
        startbuffer = 5
        addbuffer = 5
        selecting = false
    ...
    if (val != -4 && val != 32777)
    {
        if ((!menu) || scr_check_menu_repeats(input[key_select][0], val, true))
        {
            ...
            if (!is_array(val))
            {
                if (!(in.has_value((1 << 0), val)))
                    array_push(in.actions, new tdp_input_action((1 << 0), val))
            }
            else if (!(in.has_value((2 << 0), val[0], val[1])))
