function scr_collide_destructibles() //gml_Script_scr_collide_destructibles
{
    with (obj_player)
    {
        ...
        ds_list_clear(global.instancelist)
        if (vsp <= 0.5 && (state == states.jump || state == states.machcancel || state == (300 << 0) || state == states.ratmountjump || state == (301 << 0) || state == states.mach3 || state == states.machcancel || state == states.mach2 || state == states.antigrav || state == states.pogo || (state == states.bombpepup && bombup_dir == -1) || state == states.punch || state == states.climbwall || state == states.fireass || state == states.Sjump || state == (302 << 0) || state == states.cheeseballclimbwall || state == states.mach3 || state == states.machcancel || (state == states.punch && (sprite_index == spr_breakdanceuppercut || sprite_index == spr_breakdanceuppercutend))))
        {
            vy = -1
            ...
            if place_meeting(x, (y + vy), obj_destructibles)
            {
                with (instance_place(x, (y + vy), obj_destructibles))
                {
                    ...
                    with (other)
                    {
                        if (state != (302 << 0) && state != (301 << 0) && state != states.Sjump && state != states.punch && state != states.climbwall)
                            vsp = 0
